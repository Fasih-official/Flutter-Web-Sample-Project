...
class FieldAndLabelState extends State<FieldAndLabel> {
  ...
  void initState() {
    ...
    fetchLocationsByPlaceIdUseCase =
    ...
    if (widget.fieldType == FieldType.TextField ||
      ...
      _textEditController = TextEditingController(
          text: scDateFormat(currentFieldValue, format: "dd/MM/yyyy"));
      ...
      if (phone != null) {
        if ((phone ?? "").contains("-")) {
          widget.countryCode = CountryPickerUtils.getCountryByPhoneCode(
              phone.trim().split("-").first);
        }
          ...
          if (value.toString().length < 8) {
            ...
            _debounce = Timer(const Duration(milliseconds: 500), () async {
              final result = await verifyPhoneUseCase(VerifyPhoneParams(
                  phone: "+${widget.countryCode.phoneCode}-$value",
                  country: widget.country));
              result.fold((error) {
                if (currentFieldValue.toString().length > 0) {
                  setValidationMessage(error.message);
                  widget.onChanged(
                      "+${widget.countryCode.phoneCode}-$value", this);
                } else {
                  setValidationMessage("");
        ...
        if (value.toString().length <= 10) {
          if (scDateParsing(value, format: 'dd/MM/yyyy') == 1) {
            ...
            if (currentFieldValue.isAfter(widget.lastDate)) {
              ...
              setValidationMessage(
                  "Date must be before ${scDateTimeFormat(widget.lastDate, format: 'dd/MM/yyyy')}");
            } else if (currentFieldValue.isBefore(widget.firstDate)) {
              print("currentFieldValue-->$currentFieldValue");
              print("firstDate-->${widget.firstDate}");

              setValidationMessage(
                  "Date must be after ${scDateTimeFormat(widget.firstDate, format: 'dd/MM/yyyy')}");
            } else {
              setValidationMessage("");
    ...
    return Directionality(
      ...
      child: Expanded(
        child: Container(
          ...
          decoration:
              widget.boxDecoration ?? UIHelper.defaultFieldAndLabelBorder,
          child: Column(
      ...
      onKey: (key) {
        ...
        if (kIsWeb) {
          ...
          if (kReleaseMode) {
            ...
            if (kData.physicalKey.usbHidUsage == 458792 ||
                kData.physicalKey.usbHidUsage == 458979) {
            ...
            if (kData.physicalKey.debugName == "Enter" ||
                kData.physicalKey.debugName == "numpadEnter") {
              if (currentFieldValue.toString().isNotEmpty) {
                print("currentFieldValue-->${currentFieldValue}");
                onSubmitted(currentFieldValue);
                currentFieldValue = "";
                          ...
                          InkWell(
                            ...
                            child: Container(
                              ...
                              decoration: BoxDecoration(
                                  ...
                                  color: Color(
                                      widget.colorItems[index].colorCode)),
                              child: Center(
            ...
            Expanded(
              child: IgnorePointer(
                ...
                child: Tags(
                  ...
                  symmetry: false,
                  textDirection: isDirectionRTL(context) == true
                      ? TextDirection.rtl
                      : TextDirection.ltr,
...
//      height: 100.0,
                      ...
                      child: TextField(
                        ...
                        style: normalLabelTextStyle(
                            ...
                            (widget?.enabled ?? true)
                                ? regularTextColor
                                : Colors.black45),
                        textAlign: isDirectionRTL(context)
                            ? TextAlign.end
                            : TextAlign.start,
  ...
  Widget buildDatePickerField(BuildContext context) {
    widget.firstDate = widget.firstDate == null
        ? new DateTime.now().subtract(Duration(days: 365 * 100))
        : widget.firstDate;
    widget.lastDate = widget.lastDate == null
        ? new DateTime.now().add(Duration(days: 365 * 100))
        : widget.lastDate;

    DateTime defaultMinDate =
        (widget.firstDate?.isAfter(dateOnly(DateTime.now())) ?? false)
            ? widget.firstDate
                          ...
                          final List<DateTime> picked =
                              await DateRagePicker.showDatePicker(
                                  ...
                                  initialFirstDate: currentFieldValue != null
                                      ? ((widget.firstDate.isBefore(
                                                  currentFieldValue) ||
                                              widget.lastDate
                                                  .isAfter(currentFieldValue))
                                          ? (widget.lastDate
                                                  .isBefore(currentFieldValue)
                                              ? widget.lastDate
                                              : (widget.firstDate.isAfter(
                                                      currentFieldValue)
                                                  ? widget.firstDate
                                                  : currentFieldValue))
                                          : defaultMinDate)
          ...
          Expanded(
            child: InkWell(
              onTap: !widget.enabled
                  ? null
                  : () async {
                      var time = currentFieldValue == null ||
                              currentFieldValue.hour == null
                          ? TimeOfDay.now()
                          : currentFieldValue;
  ...
  Widget buildLabelField(BuildContext context) {
    return Text(
      widget.fieldValue ?? "",
      style: widget.labelValueTextStyle ??
          ...
          Expanded(
            child: TextField(
              ...
              decoration: InputDecoration(
                  ...
                  hintStyle: defaultHintStyle(null, null),
                  contentPadding:
                      EdgeInsetsDirectional.only(bottom: -10, top: 4),
                  border: InputBorder.none),
            ),
          ),
        ],
